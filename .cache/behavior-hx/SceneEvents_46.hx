	...
	override public function init()
	{
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				{
					if((((Engine.engine.getGameAttribute("Levent") : String) == "silence_musique") && !(_isstop)))
					{
						_isstop = true;
						stopAllSounds();
					}
					else if((((Engine.engine.getGameAttribute("Levent") : String) == "musique_tension") && !(_istension)))
					{
						_istension = true;
						loopSoundOnChannel(getSound(155), 0);
					}
