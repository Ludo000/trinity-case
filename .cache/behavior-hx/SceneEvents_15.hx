	...
	override public function init()
	{
			...
			if(wrapper.enabled)
			{
				...
				if(((Engine.engine.getGameAttribute("NumDialogue") : Float) <= (Engine.engine.getGameAttribute("MaxDialogue") : Float)))
				{
					...
					if(((Engine.engine.getGameAttribute("ask") : String) == "QCM_1"))
					{
						...
						g.drawString("" + "- The cannibal killed here.", (getActor(26).getX() + 20), (getActor(26).getY() + 50));
						g.drawString("" + "- It's linked to our case.", (getActor(26).getX() + 20), (getActor(26).getY() + 80));
						g.drawString("" + "- It's not linked to the case.", (getActor(26).getX() + 20), (getActor(26).getY() + 110));
						if(_HoverQCMChoix1)
						{
				...
				{
					switchScene(GameModel.get().scenes.get(14).getID(), createFadeOut(1, Utils.getColorRGB(0,0,0)), createFadeIn(1, Utils.getColorRGB(0,0,0)));
				}
			}
		});
		...
		addMouseOverActorListener(getRegion(3), function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if(((Engine.engine.getGameAttribute("ask") : String) == "QCM_1"))
				{
					...
					(Engine.engine.getGameAttribute("LineList") : Array<Dynamic>)[Std.int(((Engine.engine.getGameAttribute("NumDialogue") : Float) + 1))] = "Anna:No we can say that so early, the modu operandi is the same as the mirror killer.";
		...
		addMouseOverActorListener(getRegion(4), function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if(((Engine.engine.getGameAttribute("ask") : String) == "QCM_1"))
				{
					...
					(Engine.engine.getGameAttribute("LineList") : Array<Dynamic>)[Std.int(((Engine.engine.getGameAttribute("NumDialogue") : Float) + 1))] = "Theodore:Anyway I got more question for Mr Dubeaufeu.";
					Engine.engine.setGameAttribute("ShowPrec", -1);
					getActor(26).disableActorDrawing();
					Engine.engine.setGameAttribute("NumDialogue", ((Engine.engine.getGameAttribute("NumDialogue") : Float) + 1));
		...
		addMouseOverActorListener(getRegion(5), function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				...
				{
					playSound(getSound(224));
					(Engine.engine.getGameAttribute("LineList") : Array<Dynamic>)[Std.int(((Engine.engine.getGameAttribute("NumDialogue") : Float) + 1))] = "Anna:Yeah...I'm not an expert but I think there some chances it's related to the mirror killer...";
					Engine.engine.setGameAttribute("Perd1Pv", true);
					Engine.engine.setGameAttribute("ShowNext", false);
					Engine.engine.setGameAttribute("ShowPrec", 1);
					getActor(26).disableActorDrawing();
