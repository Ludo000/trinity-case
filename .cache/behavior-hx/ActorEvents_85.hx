...
class ActorEvents_85 extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(((Engine.engine.getGameAttribute("karma") : Float) < -500))
				{
					Engine.engine.setGameAttribute("karma", -500);
				}
				else if(((Engine.engine.getGameAttribute("karma") : Float) > 500))
				{
					Engine.engine.setGameAttribute("karma", 500);
				}
				actor.setX(((Engine.engine.getGameAttribute("pos_fleche_karma") : Float) + (Engine.engine.getGameAttribute("karma") : Float)));
			}
		});
