...
class SceneEvents_25 extends SceneScript
{
	...
	public var _compt:Float;
	public var _start:Bool;
	public var _mort:Bool;
	...
	public function new(dummy:Int, dummy2:Engine)
	{
		...
		_hoverpreuve4 = false;
		nameMap.set("compt", "_compt");
		_compt = 0.0;
		nameMap.set("start", "_start");
		_start = false;
		nameMap.set("mort", "_mort");
		_mort = false;
	...
	override public function init()
	{
		...
		hideTileLayer(engine.getLayerByName("Poison"));
			...
			if(wrapper.enabled)
			{
				if(((Engine.engine.getGameAttribute("NumDialogue") : Float) <= (Engine.engine.getGameAttribute("MaxDialogue") : Float)))
				{
					...
					if((Engine.engine.getGameAttribute("carnet_hide") : Bool))
					{
						...
						else if(((Engine.engine.getGameAttribute("ask") : String) == "launch"))
						{
							showTileLayer(engine.getLayerByName("Poison"));
							_start = true;
						}
						else if(((Engine.engine.getGameAttribute("ask") : String) == "QCM_1"))
						{
							...
							else if(_HoverQCMChoix3)
							{
								g.fillColor = Utils.convertColor(Utils.getColorRGB(0,0,0));
								g.strokeSize = 5;
								g.drawRect(getRegion(15).getX(), getRegion(15).getY(), getRegion(15).getWidth(), getRegion(15).getHeight());
							}
						}
						else
						{
						...
						if((Engine.engine.getGameAttribute("Q1") : Bool))
						{
							...
							{
								(Engine.engine.getGameAttribute("LineList") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("NumDialogue") : Float))] = " Theodore : So, i think the murderer is simply Kevin the roxxor, the cannibal who is related with you by this old case, who was found with you in the cave 20 years ago.";
							}
							else if(((Engine.engine.getGameAttribute("QKarma") : String) == "bad"))
							{
								(Engine.engine.getGameAttribute("LineList") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("NumDialogue") : Float))] = "Theodore : Stop joking with me ! This murder is the cannibal who was with you in this cave, where you attacked us, me and my coworker, and he also was foud with you in this old case 20 years ago.";
							}
						}
					}
					else
					{
						...
						else if(((Engine.engine.getGameAttribute("page_carnet") : Float) == 2))
						{
							Engine.engine.setGameAttribute("ShowFiche", false);
							g.setFont(getFont(26));
							g.drawString("" + "Poison found near me.", (scripts.SceneEvents_3._customBlock_X_to_the_right_of(getActor(34)) : Float), (scripts.SceneEvents_3._customBlock_Y_to_the_right_of(getActor(34), 0) : Float));
							if(_hoverpreuve1)
							{
								g.fillColor = Utils.convertColor(Utils.getColorRGB(0,0,0));
								g.strokeSize = 5;
								g.drawRect((getActor(34).getX() - 10), (getActor(34).getY() - 10), 500, ((getActor(34).getHeight()) + 20));
							}
						}
						else if(((Engine.engine.getGameAttribute("page_carnet") : Float) == 3))
						{
							Engine.engine.setGameAttribute("ShowFiche", false);
							g.setFont(getFont(25));
							g.drawString("" + "bn", (getActor(12).getX() + 380), (getActor(12).getY() + 100));
							g.setFont(getFont(26));
							_CurrentLine = 0;
							_linelenght = 0;
				...
				{
					switchScene(GameModel.get().scenes.get(26).getID(), createFadeOut(1, Utils.getColorRGB(0,0,0)), createFadeIn(1, Utils.getColorRGB(0,0,0)));
				}
			}
		});
		...
		addMouseOverActorListener(getRegion(13), function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if(((Engine.engine.getGameAttribute("ask") : String) == "QCM_1"))
				{
					...
					(Engine.engine.getGameAttribute("LineList") : Array<Dynamic>)[Std.int(((Engine.engine.getGameAttribute("NumDialogue") : Float) + 1))] = "Theodore : Okay I think I got the right one.";
					Engine.engine.setGameAttribute("ShowPrec", -1);
					_QCM1 = true;
					Engine.engine.setGameAttribute("ShowNext", true);
					Engine.engine.setGameAttribute("NumDialogue", ((Engine.engine.getGameAttribute("NumDialogue") : Float) + 1));
		...
		addMouseOverActorListener(getRegion(14), function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if(((Engine.engine.getGameAttribute("ask") : String) == "QCM_1"))
				{
					playSound(getSound(224));
					Engine.engine.setGameAttribute("ask", "null");
					(Engine.engine.getGameAttribute("LineList") : Array<Dynamic>)[Std.int(((Engine.engine.getGameAttribute("NumDialogue") : Float) + 1))] = "Theodore : Arghh ....";
					Engine.engine.setGameAttribute("ShowPrec", -1);
					Engine.engine.setGameAttribute("ShowNext", false);
					Engine.engine.setGameAttribute("NumDialogue", ((Engine.engine.getGameAttribute("NumDialogue") : Float) + 1));
					_mort = true;
		...
		/* ========================== On Region =========================== */
		addMouseOverActorListener(getRegion(15), function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if(((Engine.engine.getGameAttribute("ask") : String) == "QCM_1"))
