...
public var _isstop:Bool;

public var _istension:Bool;
...
nameMap.set("isstop", "_isstop");
_isstop = false;
nameMap.set("istension", "_istension");
_istension = false;
...
/* ======================== When Creating ========================= */
        ...
        _isstop = false;
propertyChanged("_isstop", _isstop);
        _istension = false;
propertyChanged("_istension", _istension);
...
{
            if(((Engine.engine.getGameAttribute("Levent") == "silence_musique") && !(_isstop)))
{
                _isstop = true;
propertyChanged("_isstop", _isstop);
                stopAllSounds();
            ...
            else if(((Engine.engine.getGameAttribute("Levent") == "musique_tension") && !(_istension)))
{
                _istension = true;
propertyChanged("_istension", _istension);
                loopSoundOnChannel(getSound(155), Std.int(0));
}
...
{
            switchScene(GameModel.get().scenes.get(47).getID(), createFadeOut(1, Utils.getColorRGB(0,0,0)), createFadeIn(1, Utils.getColorRGB(0,0,0)));
}
