...
propertyChanged("_HoverQCMChoix2", _HoverQCMChoix2);
        _hadshot = false;
propertyChanged("_hadshot", _hadshot);
        _HoverQCMChoix1 = false;
...
{
            if((Engine.engine.getGameAttribute("PersoFeel") == "gentil"))
{
                getActor(29).setAnimation("" + "gentil");
}

            else if((Engine.engine.getGameAttribute("PersoFeel") == "colere"))
{
                getActor(29).setAnimation("" + "colere");
}

            else if((Engine.engine.getGameAttribute("PersoFeel") == "doute"))
{
                getActor(29).setAnimation("" + "doute");
}

            else if((Engine.engine.getGameAttribute("PersoFeel") == "normal"))
{
                getActor(29).setAnimation("" + "normal");
}

            if((Engine.engine.getGameAttribute("Levent") == "content"))
{
                getActor(31).setAnimation("" + "content");
}

            else if((Engine.engine.getGameAttribute("Levent") == "colere"))
{
                getActor(31).setAnimation("" + "colere");
}

            else if((Engine.engine.getGameAttribute("Levent") == "mort"))
{
                getActor(31).setAnimation("" + "mort");
                if(!(_hadshot))
{
...
{
                getActor(31).setAnimation("" + "normal");
}
            ...
            if((Engine.engine.getGameAttribute("ask") == "QCM_1"))
{
                ...
                g.drawString("" + "- Tell the other.", (getActor(28).getX() + 20), (getActor(28).getY() + 50));
                g.drawString("" + "- Stay silent.", (getActor(28).getX() + 20), (getActor(28).getY() + 80));
                if(_HoverQCMChoix1)
{
                    g.fillColor = Utils.getColorRGB(0,0,0);
                    g.strokeSize = Std.int(5);
                    g.drawRect((getActor(28).getX() + 15), (getActor(28).getY() + 45), 600, 30);
}
                ...
                else if(_HoverQCMChoix2)
{
                    g.fillColor = Utils.getColorRGB(0,0,0);
                    g.strokeSize = Std.int(5);
                    g.drawRect((getActor(28).getX() + 15), (getActor(28).getY() + 75), 600, 30);
}

}

            else if((Engine.engine.getGameAttribute("ask") == "pula"))
{
                getActor(31).disableActorDrawing();
}

            else if((Engine.engine.getGameAttribute("ask") == "ouch"))
{
                startShakingScreen(0.5 / 100, 0.2);
}
...
{
            switchScene(GameModel.get().scenes.get(45).getID(), createFadeOut(2, Utils.getColorRGB(0,0,0)), createFadeIn(2, Utils.getColorRGB(0,0,0)));
}
        ...
        if((Engine.engine.getGameAttribute("ask") == "QCM_1"))
{
            ...
            Engine.engine.getGameAttribute("LineList")[Std.int((Engine.engine.getGameAttribute("NumDialogue") + 1))] = "Blanche : Well if you take it that way ... : colere : reload";
...
{
            playSound(getSound(224));
            Engine.engine.getGameAttribute("LineList")[Std.int((Engine.engine.getGameAttribute("NumDialogue") + 1))] = "Blanche : Good i'm glad to see we understand each other, it's so rare these days... : normal";
            Engine.engine.setGameAttribute("ShowPrec", -1);
            Engine.engine.setGameAttribute("ShowNext", true);
            getActor(28).disableActorDrawing();
            Engine.engine.setGameAttribute("NumDialogue", (Engine.engine.getGameAttribute("NumDialogue") + 1));
